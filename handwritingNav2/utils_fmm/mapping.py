...
class Semantic_Mapping(nn.Module):
    ...
    def __init__(self, config, device, map_size_cm, map_resolution, max_height=None, min_height=None, num_cats=1):
        ...
        # self.screen_h = config.SIMULATOR.DEPTH_SENSOR.HEIGHT
        # self.screen_w = config.SIMULATOR.DEPTH_SENSOR.WIDTH
        # self.agent_height = config.SIMULATOR.AGENT_0.HEIGHT*100.
        self.resolution = map_resolution
        self.z_resolution = 5
        self.map_size_cm = map_size_cm
        self.vision_range = 100  # 增大视野范围，原值为100
        self.dropout = 0.5
        self.fov = config.TASK_CONFIG.SIMULATOR.DEPTH_SENSOR.HFOV
        # self.fov = config.SIMULATOR.DEPTH_SENSOR.HFOV
        self.du_scale = 1
    ...
    def forward(self, depth, pose_obs, maps_last, type_mask=None, type_prob=None):
        ...
        # z_cam = point_cloud_t[..., 2]          # 相机坐标系下 z（距离相机向前）
        # print('camera z  min/mean/max:',
        #       z_cam.min().item(),
        #       z_cam.mean().item(),
        #       z_cam.max().item())   
        #Multiprocessing
        ...
        # z_agent = agent_view_t[..., 2]         # 现在 z≈“离地高度(cm)”
        # print('agent  z  min/mean/max:',
        #       z_agent.min().item(),
        #       z_agent.mean().item(),
        #       z_agent.max().item())
        agent_view_centered_t = du.transform_pose_t(agent_view_t, self.shift_loc, self.device)
        
        # z_grid = agent_view_centered_t[..., 2]
        # print('grid   z  min/mean/max:',
        #       z_grid.min().item(),
        #       z_grid.mean().item(),
        #       z_grid.max().item())
