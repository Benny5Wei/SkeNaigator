...
class HandWritingNavRLEnv(habitat.RLEnv):
    ...
    def get_info(self, observations):
        ...
        # 如果启用了专家动作，在info中添加专家动作
        if self._use_expert_actions:
            if self._path_follower is None:
                # 如果_path_follower未初始化，尝试重新初始化
                logging.warning("ShortestPathFollower未初始化，尝试重新初始化")
                try:
                    from habitat.tasks.nav.shortest_path_follower import ShortestPathFollower
                    self._path_follower = ShortestPathFollower(
                        self._env.sim,
                    ...
                    logging.warning("ShortestPathFollower重新初始化成功")
                except Exception as init_error:
                    logging.error(f"ShortestPathFollower初始化失败: {init_error}")
                    # 提供默认动作并返回
                    info["expert_action"] = 0
                    return info
            
            try:
                # 打印 NavMesh 状态
                has_navmesh = hasattr(self._env.sim, 'pathfinder') and self._env.sim.pathfinder.is_loaded
                logging.debug(f"NavMesh loaded: {has_navmesh}")
                
                # 获取当前智能体状态和目标位置
                agent_state = self._env.sim.get_agent_state()
                current_position = agent_state.position
                logging.debug(f"当前位置: {current_position}")
                
                # 获取当前episode的目标位置
                goal_position = self.habitat_env.current_episode.goals[0].position
                logging.debug(f"目标位置: {goal_position}")
                
                # 使用ShortestPathFollower计算最优动作
                expert_action = self._path_follower.get_next_action(
                    goal_position
                )
                # 将专家动作添加到info字典中
                info["expert_action"] = expert_action
                logging.debug(f"生成的专家动作: {expert_action}")
                
            except Exception as e:
                # 如果计算失败，详细记录错误并使用默认动作
                logging.error(f"计算专家动作失败: {e}")
                # 使用STOP作为默认专家动作
                info["expert_action"] = 0
